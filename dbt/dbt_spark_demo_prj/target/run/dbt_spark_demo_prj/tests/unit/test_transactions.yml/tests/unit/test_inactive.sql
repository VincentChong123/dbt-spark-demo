...
with dbt_internal_unit_test_actual as (
  select
    id,amount,currency,amount_usd,branch_marker, 'actual' as `actual_or_expected`
  from (
...
select cast(2 as bigint)
 ...
 as currency, cast('INACTIVE' as string)
 as status
), base AS (
    SELECT
        id,
...
dbt_internal_unit_test_expected as (
  select
    id, amount, currency, amount_usd, branch_marker, 'expected' as `actual_or_expected`
  from (
